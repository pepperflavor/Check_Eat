...
module.exports = async function seedVeganGangnam(prisma) {
  ...
  const rows = STORES.map((s) => {
    const bs_no = 'PENDING_VEG_' + esc(s.sto_name_en).toUpperCase().replace(/[^A-Z0-9]+/g, '_');
...
`;
  ...
  try {
    TranslateServiceClass = require('../dist/translate/translate.service').TranslateService;
    ConfigServiceClass = require('@nestjs/config').ConfigService;
    console.log('🔗 TranslateService(dist) 연결됨');
  } catch {
    try {
      TranslateServiceClass = require('../src/translate/translate.service').TranslateService;
      ConfigServiceClass = require('@nestjs/config').ConfigService;
  ...
  function createTranslateWrapper() {
    ...
    return {
      ...
      async array(inputs, toArr, from) {
        const url = `${EP}/translate`;
        if (!Array.isArray(inputs) || inputs.length === 0) return Object.fromEntries(toArr.map((t) => [t, []]));
        ...
        const out = {}; toArr.forEach((l) => (out[l] = []));
        for (const row of rows) {
          const map = new Map((row.translations || []).map((t) => [t.to, t.text]));
  ...
  try { ({ AzureOpenAI } = await import('openai')); } catch {
...
`; }
  function veganJudgeSchema() {
    return {
      ...
      properties: {
        ...
        matched: { type: 'object', additionalProperties: { type: 'array', items: { type: 'string' } } },
  ...
  async function judgeVeganByIngredientsLLM(ingredients) {
    if (!AzureOpenAI) return { veg_id: null, matched: {}, reasoning: 'no-openai' };
    try {
      ...
      const r = await mini.chat.completions.create({
        messages: [
          ...
          { role: 'user', content: `재료 목록(JSON 배열): ${JSON.stringify(ingredients)}` },
        ],
        model: modelName,
        temperature: 0.0,
        max_tokens: 400,
        response_format: {
          type: 'json_schema',
          json_schema: { name: 'VeganJudge', schema: veganJudgeSchema(), strict: true },
        },
      });
  ...
  async function veganIdIfExists(id) {
    ...
    const v = await prisma.vegan.findUnique({ where: { veg_id: id }, select: { veg_id: true } });
    return v ? id : null;
  }
  function splitKoEn(nameWithParen) {
    const m = nameWithParen.match(/(.+?)\s*\((.*?)\)/);
    return m ? { ko: m[1].trim(), en: m[2].trim() } : { ko: nameWithParen.trim(), en: null };
  }

  // ── (A) PHONE 매핑 ──────────────────────────────────────────
  const PHONE = Object.fromEntries(STORES.map(s => {
  ...
  const MENU = [
    {
      ...
      items: [
        { name: '페이퍼 두부 라자냐 (Paper Tofu Lasagna)', price: 16900, materials: ['두부','토마토소스','양파','마늘','바질','후추','올리브오일'] },
    ...
    },
    {
      phone: PHONE['NORDI'],
      items: [
        { name: '비건 크루아상 (Vegan Croissant)', price: 6000, materials: ['밀가루','비건마가린','이스트','소금','설탕','물'] },
        { name: '비건 티라미수 (Vegan Tiramisu)',  price: 8500, materials: ['두유크림','에스프레소','설탕','코코아파우더','비건비스킷'] },
      ],
    },
    {
    ...
    },
    {
      ...
      items: [
        { name: '부아베스 스타일 토마토 스튜 (Vegan Bouillabaisse)', price: 23000, materials: ['토마토','향신채','허브','올리브오일','채수'] },
        { name: '머쉬룸 크림 파스타 (Mushroom Cream Pasta)',         price: 24000, materials: ['파스타(글루텐프리 선택)','버섯','두유크림','마늘','올리브오일'] },
      ],
    },
    {
      phone: PHONE['AN_SIK_RESTAURANT'],
      items: [
        { name: '나물 듬뿍 크림 파스타 (Greens Cream Pasta)', price: 19000, materials: ['파스타','나물','두유크림','마늘','소금','후추'] },
        { name: '세상의 모든 버섯 샐러드 (All the Mushrooms Salad)', price: 17000, materials: ['각종버섯','채소','올리브오일','발사믹','소금'] },
      ],
    },
    {
      phone: PHONE['CHICKPEACE_SINSA_GAROSU_GIL'],
      items: [
        { name: '팔라펠 피타 (Falafel Pita)',             price: 8900,  materials: ['병아리콩','파슬리','양파','마늘','피타','토마토','양상추','타히니'] },
        { name: '콜리플라워 로스트 보울 (Cauliflower Roast Bowl)', price: 10900, materials: ['콜리플라워','현미밥','각종채소','타히니','향신료'] },
      ],
    },
  ].filter(b => !!b.phone);
  ...
  for (const block of MENU) {
    ...
    if (!store) { console.log(`⚠️ Store 미발견(phone=${block.phone}) — 건너뜀`); continue; }
    ...
    for (const item of block.items) {
      ...
      if (dup) { console.log(`  ↪️ 이미 존재: [${store.sto_name}] ${nameKo}`); continue; }
      ...
      } catch (e) { console.log(`   ⚠️ 재료 번역 실패(${nameKo}):`, e?.message || e); }

      // 4) 이름 번역
      let enName = nameEnFromTitle, arName = null;
      try {
        if (!enName) {
      ...
      } catch (e) { console.log(`   ⚠️ 이름 번역 실패(${nameKo}):`, e?.message || e); }

      // 5) 번역 테이블 저장
      try {
        await prisma.foodTranslateEN.upsert({
          where: { food_id: created.foo_id },
          create: { food_id: created.foo_id, ft_en_name: enName || null, ft_en_mt: enMt, ft_en_price: String(item.price) },
          update: { ft_en_name: enName || null, ft_en_mt: enMt, ft_en_price: String(item.price) },
        });
        await prisma.foodTranslateAR.upsert({
          where: { food_id: created.foo_id },
          create: { food_id: created.foo_id, ft_ar_name: arName || null, ft_ar_mt: arMt, ft_ar_price: String(item.price) },
          update: { ft_ar_name: arName || null, ft_ar_mt: arMt, ft_ar_price: String(item.price) },
        });
      } catch (e) { console.log(`   ⚠️ 번역 저장 실패(${nameKo}):`, e?.message || e); }

      console.log(`  ✅ [${store.sto_name}] ${nameKo}${enName ? ` (${enName})` : ''} - ₩${item.price.toLocaleString()}${veganId ? ` | vegan_id=${veganId}` : ''}`);
    }
  }
