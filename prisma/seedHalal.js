...
// 외부 시드 모듈 (있으면 사용)
let seedStores = async () => console.log('ℹ️ seedStores 모듈이 없어 건너뜀');
let seedFoods = async () => console.log('ℹ️ seedFoods 모듈이 없어 건너뜀');
try {
  seedStores = require('./seedStores');
} catch (_) {}
try {
  seedFoods = require('./seedFoods');
} catch (_) {}
...
try {
  ...
  console.log('⚠️ gangnam_restaurants.json 없음 → 빈 배열로 진행');
  restaurantData = [];
}
...
/** ─────────────────────────────────────────────────────────
 *  [추가] 용산구 할랄 7개 업장 시드 SQL (BEGIN/COMMIT 제거본)
 *  ───────────────────────────────────────────────────────── */
const HALAL_SQL = `
-- 1) 식당 원천 데이터 (좌표는 보정값 사용)
WITH venues AS (
  SELECT * FROM (VALUES
    -- bs_no                , name_en                                   , name_kor                         , address_kor                                   , phone           , lat        , lng
...
-- 2) 식당 수(7개)만큼 Sajang 생성/유지 (전화번호를 자연키로 사용)
upsert_sajang AS (
...
-- 3) BusinessCerti 더미 생성/유지 (bs_no 유니크) + 각 Sajang 1:1 연결
upsert_biz AS (
...
-- 4) Store upsert (모두 '음식점' 고정, sto_halal=1)
INSERT INTO "Store"(
...
ON CONFLICT ON CONSTRAINT "uniq_store_owner_cert_name_geo"
DO UPDATE SET
...
async function ensureOwnersSeed(total = 10) {
  ...
  for (let i = 1; i <= total; i++) {
    ...
    // 사장 계정 생성 (인증 상태는 필요시 1,1로 통일 가능)
    const sajang = await prisma.sajang.create({
      data: {
        sa_phone: `010-1234-${String(5600 + i).padStart(4, '0')}`,
        sa_certification: i % 2 === 0 ? 1 : 0,
        sa_certi_status: i % 2, // 짝수: 1(완료), 홀수: 0(대기)
      },
    });
...
async function main() {
  ...
  await ensureOwnersSeed(10);

  // ↙️ 여기서 용산 할랄 7개 업장 시드 실행
  await runHalalYongsanSeed();

  console.log('🏪 Store 시드 시작');
  await seedStores({ prisma, restaurantData }).catch((e) => {
    console.log('⚠️ seedStores 실행 중 경고:', e?.message || e);
  });

  console.log('🍽️ Food 시드 시작');
  await seedFoods({ prisma }).catch((e) => {
    console.log('⚠️ seedFoods 실행 중 경고:', e?.message || e);
  });
